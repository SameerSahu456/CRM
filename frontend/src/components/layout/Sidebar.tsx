...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      <aside className={`fixed top-0 left-0 z-50 h-[100dvh] flex flex-col overflow-hidden sidebar-premium border-r transition-all duration-300 ${theme === 'dark' ? 'border-white/[0.06]' : 'border-slate-200'
        } ${isOpen
        ...
        {/* Brand */}
        <div className={`h-14 sm:h-16 flex-shrink-0 flex items-center justify-between px-4 sm:px-5 border-b ${theme === 'dark' ? 'border-white/[0.06]' : 'border-slate-200'
          }`}>
          <button
          ...
          >
            <div className="comprint-logo text-[22px]">
              ...
              <span className={`comprint-dot ${!isOpen ? 'md:ml-0' : ''} ${theme === 'dark' ? 'bg-brand-400 shadow-[0_0_10px_rgba(129,140,248,0.5)]' : 'bg-slate-900 shadow-sm'
                }`} />
            </div>
          </button>
          <button onClick={onClose} className={`md:hidden p-1.5 rounded-lg transition-colors ${theme === 'dark' ? 'text-zinc-500 hover:text-white hover:bg-zinc-800' : 'text-slate-400 hover:text-slate-600 hover:bg-slate-100'
            }`}>
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className={`px-3 py-3 flex-shrink-0 border-b ${!isOpen ? 'hidden lg:block' : ''} ${theme === 'dark' ? 'border-white/[0.06]' : 'border-slate-200'
          }`}>
          <ViewSwitcher />
        </div>
          ...
          {sections.map(section => {
            ...
            return (
              <div key={section} className="mb-4">
                ...
                <p className={`px-3 mb-1 text-[10px] font-semibold uppercase tracking-wider ${!isOpen ? 'hidden lg:block' : ''
                ...
                {sectionItems.map(item => (
                  <button
                    ...
                    title={item.label}
                    className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-medium transition-all active:scale-[0.98] ${!isOpen ? 'md:justify-center lg:justify-start' : ''
                      } ${activeTab === item.id
                        ? theme === 'dark'
                  ...
                  >
                    ...
                    {activeTab === item.id && (
                      <span className={`ml-auto w-1.5 h-1.5 rounded-full ${!isOpen ? 'hidden lg:block' : ''} ${theme === 'dark' ? 'bg-brand-400 shadow-[0_0_6px_rgba(129,140,248,0.6)]' : 'bg-brand-600'
                        }`} />
                    )}
                  </button>
                ))}
        ...
        <div className={`flex-shrink-0 p-3 sm:p-4 border-t safe-area-bottom ${theme === 'dark' ? 'border-white/[0.06]' : 'border-slate-200'}`}>
          {user && (
            <div className={`flex items-center gap-3 mb-3 px-2 ${!isOpen ? 'md:justify-center lg:justify-start' : ''} ${theme === 'dark' ? 'text-zinc-400' : 'text-slate-600'
              }`}>
          ...
          <button
            onClick={signOut}
            title="Sign Out"
            className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl text-sm transition-all ${!isOpen ? 'md:justify-center lg:justify-start' : ''
              } ${theme === 'dark'
