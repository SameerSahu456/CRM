...
async def _ensure_schema() -> None:
    """Ensure all columns/tables exist on production DB (idempotent).

    Runs once per cold start. Uses ADD COLUMN IF NOT EXISTS so it's safe
    to run repeatedly.
    """
...
async def ensure_schema_middleware(request, call_next):
    """Run schema migration on first request (needed for Vercel serverless)."""
